...
public class Flashlight extends CordovaPlugin {
  ...
  private static final String ACTION_SWITCH_ON_PREP = "switchOnPrep";
  private static final String ACTION_SWITCH_OFF_PREP = "switchOffPrep";
  private static final String ACTION_SWITCH_ON_FAST = "switchOnFast";
  private static final String ACTION_SWITCH_OFF_FAST = "switchOffFast";
  ...
  public boolean execute(final String action, final JSONArray args, final CallbackContext callbackContext) throws JSONException {
    ...
    try {
      if (action.equals(ACTION_SWITCH_ON)) {
        ...
        cordova.getThreadPool().execute(new Runnable() {
          public void run() {
            ...
            if (!hasPermisssion()) {
              ...
              toggleTorchPrep(true);
            }
          }
        });

        return true;
      } else if (action.equals(ACTION_SWITCH_OFF_PREP)) {
        cordova.getThreadPool().execute(new Runnable() {
          public void run() {
            toggleTorchPrep(false);
            releaseCamera();
          }
        });
        return true;
      } else if (action.equals(ACTION_SWITCH_ON_FAST)) {
        ...
        return true;
      } else if (action.equals(ACTION_SWITCH_OFF_FAST)) {
        cordova.getThreadPool().execute(new Runnable() {
          public void run() {
            toggleTorchFast(false);
          }
        });
        return true;
  ...
  private void toggleTorchPrep(boolean switchOn) {
    try {
      if (isCapable()) {
        doToggleTorchPrep(switchOn);
      } else {
        callbackContext.error("Device is not capable of using the flashlight. Please test with flashlight.available()");
      }
    } catch (Exception e) {
  ...
  private void doToggleTorchPrep(boolean switchOn) throws IOException, CameraAccessException {
    if (Build.VERSION.SDK_INT >= 23) { // Android M has such an easy API! <3
      doToggleTorchSdk23Prep(switchOn);
  ...
  private void doToggleTorchFast(boolean switchOn) throws IOException, CameraAccessException {
    if (Build.VERSION.SDK_INT >= 23) { // Android M has such an easy API! <3
      doToggleTorchSdk23Fast(switchOn);
  ...
  private void doToggleTorchSdk23Prep(boolean switchOn) throws IOException, CameraAccessException {
    final CameraManager cameraManager = (CameraManager) cordova.getActivity().getSystemService(Context.CAMERA_SERVICE);
    for (final String id : cameraManager.getCameraIdList()) {
      // Turn on the flash if the camera has one (usually the one at index 0 has one)
      final Boolean hasFlash = cameraManager.getCameraCharacteristics(id).get(CameraCharacteristics.FLASH_INFO_AVAILABLE);
  ...
  private void doToggleTorchSdk23Fast(boolean switchOn) throws IOException, CameraAccessException {
        setTorchMode(mCameraManager, mId, switchOn);
  }
